...
| `socks` | Необязательный SOCKS5-прокси для SSH в виде `host:port` (например, туннель `ssh -D` / autossh). Нужен, когда хост доступен только через прокси. Взаимоисключающе с `jump` |
| `jump` | Имя другого сервера из `servers`, через который проксируется SSH-подключение (ProxyJump). Нужен, когда хост достижим только через бастион/промежуточный узел. Взаимоисключающе с `socks` |
...
| `muted` | Список имён сервисов, скрытых из статуса и алёртов (заполняется кнопкой mute в UI) |
...
Подробнее об архитектуре модулей, устройстве темы и регенерации скриншотов — в [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md).
